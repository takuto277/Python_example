...
#
        self.size = 200
        self.clock = tk.Canvas(self, width=self.size,
...
12 - 90))*self.size/2*0.85
            self.clock.create_text(x,y,text=str(number),
 fill="black", font =("",14))
#
#  日付表示をオンオフするボタンの作成
#
        self.b = tk.Button(self, text="Show Date",
 font=("",14), command = self.toggle)
...
#
 ...
 tag="SEC")
        ...
        self.clock.create_line(x0,y0,x,y, width=3, fill="blue",
 tag="MIN")

        self.hour = time.localtime().tm_hour
        x0 = self.size/2
        y0 = self.size/2
        angle = math.radians((self.hour%12+self.min/60)*360/12 
- 90)
        x = self.size/2 + math.cos(angle)*self.size/2*0.55
        y = self.size/2 + math.sin(angle)*self.size/2*0.55
        self.clock.delete("HOUR")
        self.clock.create_line(x0,y0,x,y, width=3, fill="green"
, tag="HOUR")
        #
        ...
        if self.show_date:
            self.clock.create_text(x, y, text=text, font=("",12),
 fill="black", tag="TIME")
        #
        # 100 ミリ秒後に再度呼び出す
        # 
